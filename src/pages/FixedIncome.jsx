            ...
            <header className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3">
                        <TrendingUp className="text-emerald-400" size={32} /> Rendimentos Fixos
                    </h1>
                    <p className="text-slate-400 mt-2 text-lg">Gerencie seus ganhos mensais recorrentes.</p>
                </div>
                <button onClick={() => { setEditingId(null); setFormVisible(true); }} className="bg-emerald-500 hover:bg-emerald-600 text-white px-5 py-3 rounded-xl font-bold shadow-lg shadow-emerald-500/20 flex items-center gap-2 transition-transform hover:scale-105 active:scale-95 group">
                    <Plus size={20} className="group-hover:rotate-90 transition-transform duration-300" /> <span className="hidden sm:inline">Novo Rendimento</span>
                </button>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {income.map((item) => {
                    const isRegistered = statusMap[item.id];
                    return (
                        <div key={item.id} className={`bg-slate-900 border border-slate-800 p-6 rounded-2xl relative group transition-all hover:bg-slate-800/50 ${isRegistered ? 'opacity-75' : 'hover:border-emerald-500/30'}`}>
                            <div className="flex justify-between items-start mb-4">
                                <div className={`p-3 rounded-xl ${isRegistered ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-500/10 text-emerald-400'}`}>
                                    <TrendingUp size={24} />
                                </div>
                                <div className="flex gap-2">
                                    <span className="text-xs font-bold px-2 py-1 bg-slate-800 rounded-md text-slate-400 border border-slate-700 h-fit">Dia {item.day}</span>
                                    <button onClick={() => handleEdit(item)} className="p-1 hover:bg-slate-700 text-slate-500 hover:text-white rounded-lg transition-colors" title="Editar">
                                        <Pencil size={16} />
                                    </button>
                                    <button onClick={() => handleDelete(item.id)} className="p-1 hover:bg-rose-500/20 text-slate-600 hover:text-rose-500 rounded-lg transition-colors" title="Excluir">
                                        <Trash2 size={16} />
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center justify-between mt-6">
                                ...
                                <button
                                    onClick={() => isRegistered ? handleUnregister(item.id) : handleRegister(item)}
                                    className={`px-4 py-2 rounded-lg font-bold text-sm transition-all flex items-center gap-2 ${isRegistered
                                        ...
                                        : 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-lg shadow-emerald-500/20 hover:scale-105 active:scale-95'
                                        }`}
                                    title={isRegistered ? "Clique para estornar/remover o registro deste mês" : "Confirmar recebimento deste mês"}
                                >
                                    {isRegistered ? 'Recebido (Desfazer)' : 'Confirmar Recebimento'}
                                </button>
                            </div>

                            <div className={`absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500/20 via-emerald-500/50 to-emerald-500/20 opacity-0 transition-opacity ${isRegistered ? '' : 'group-hover:opacity-100'}`}></div>
                        </div>
                    );
                })}
                {income.length === 0 && (
                    <div className="col-span-full py-12 text-center text-slate-500 border border-dashed border-slate-800 rounded-3xl bg-slate-900/20">
                        ...
                        <p className="text-lg">Nenhum rendimento fixo cadastrado.</p>
                    </div>
                )}
            </div>
            ...
            {formVisible && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-3xl p-8 shadow-2xl animate-scale-in">
                        <h2 className="text-2xl font-bold text-white mb-6">{editingId ? 'Editar Rendimento' : 'Novo Rendimento'}</h2>
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div>
                                <label className="block text-slate-400 text-sm font-bold mb-2 ml-1">Nome</label>
                                <input required autoFocus type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white focus:border-emerald-500/50 outline-none transition-colors" placeholder="Ex: Salário" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-slate-400 text-sm font-bold mb-2 ml-1">Dia do Receb.</label>
                                    <input required type="number" min="1" max="31" value={formData.day} onChange={e => setFormData({ ...formData, day: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white focus:border-emerald-500/50 outline-none transition-colors" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-slate-400 text-sm font-bold mb-2 ml-1">Categoria</label>
                                <input list="categories-income" type="text" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })} className="w-full bg-slate-950 border border-slate-800 rounded-xl p-4 text-white focus:border-emerald-500/50 outline-none transition-colors placeholder:text-slate-700" placeholder="Ex: Trabalho" />
                            </div>
